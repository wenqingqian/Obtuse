...
\subsection{ISOK:\color{purple}{{\tt C}和{\tt C++}区别}}
...
\end{enumerate}
\subsection{ISOK:\color{purple}{{\tt C++}从代码到可执行二进制文件的过程}}
\begin{enumerate}
	...
	\end{itemize}
\end{enumerate}
\subsection{ISOK:\color{purple}{{\tt static}关键字}}
\begin{enumerate}
...
\subsection{ISOK:\color{purple}{{\tt extern}关键字与链接性}}
...
\subsection{ISOK:\color{purple}{指针与引用}}
...
测试程序见\href{https://github.com/wenqingqian/Obtuse/tree/main/test/cpp/basic/pointer_1}{$\mathbb{CPP\_BASIC\_POINTER\_1}$}
...
\subsection{ISOK:\color{purple}{\tt{decltype}和\tt{auto}}}
...
\subsection{ISOK:\color{purple}{左右值}}
\subsubsection{左值与右值}
\href{https://zhuanlan.zhihu.com/p/343507680}{原文}
...
\end{enumerate}
参考代码见\href{https://github.com/wenqingqian/Obtuse/tree/main/test/cpp/c++11/copy_0}{$\mathbb{CPP\_C11\_COPY\_0}$}
...
C++11直接统一, 增加\&\&右值引用, 事情变简单, 想要深拷贝的就T(const T\&)实现;
	想要移动拷贝的就实现T(T\&\&). 两者都实现时, 一旦传入是个右值, 优先触发移动拷贝构造调用, 
	使用移动语意, 转移资源, 减少拷贝

\subsubsection{万能引用与引用折叠}
在auto那一章提到过万能引用, 是以auto\&\&的形式.
...
它表明参数是一个引用, 但未必是右引用, 因为C++的引用折叠机制
\begin{itemize}
	\item T\&  \& 折叠为T\&
	\item T\&\& \& 折叠为T\&
	\item T\&  \&\&折叠为T\&
...
当传入为左值T=int\&, 根据引用折叠机制int\& \&\&=int\&; 当传入为右值,T=int.(万能引用处理右值时, T为int而不是T\&\&)

万能引用可以接受左值和右值, 但具有如下规则:
\begin{enumerate}
	\item 万能引用的T不能被再修饰, 不能被cv修饰限定, 否则转为普通右值引用
...
template <class _Tp> struct remove_reference<_Tp&&> {typedef _Tp type;};
\end{lstlisting}
将类型转化为remove\_reference::type时, 根据原类型自动选择合适的版本并得到去掉引用后的类型. 一般是在模板
...
函数原型:
\begin{lstlisting}[title=forward,xleftmargin=2em,xrightmargin=2em]
	//处理左值作为左引用或者右引用
	template <class _Tp>
	_Tp&&
...
2. T=int, return forward<int>(t) == return static_cast<int&&>(t)
3. 由上可知, forward跟static_cast<T&&>的作用是一样的, 原类型是左引用返回左引用, 原类型右引用返回右引用*/
\end{lstlisting}
在万能模板情况下, t可能是左引用也可能是右引用, 但一定是左值, 无法再被右引用.

\begin{enumerate}
	\item t为右值, 则函数返回右值.
	\item t为左值, 原类型为右值引用, 即实例模板参数T输入为非引用类型, 则返回右值, 其余返回左值
\end{enumerate}

\noindent 保持原来的左右引用关系并向下层次转化, 不再因右引用表达式是左值从而变掉.

特殊情况: 传入的T是左引用, 传入的参数为右值, 即返回对右值的左引用.
\vspace*{2em}
\begin{lstlisting}[xleftmargin=2em,xrightmargin=2em]
...
\end{lstlisting}
当传入参数与模板实例T不是同一类别时, 且存在两个类别的转换, 则会在forward<T1>(a2)时将TypeB a2通过构造函数
	生成一个临时的TypeA变量并传入forward中, 这个临时变量为右值, 因此会触发forward版本2, 而T1为const TypeA\&, 
	会返回对右值的左引用, 因此forward分开两个版本并在右值处理中判断T是否为左引用. 除此之外与static\_cast<T\&\&>无差别.

\subsection{\tt{using}}

\subsubsection{作用域导入}
“把目标的访问权限复制到using所在的当前作用域”, 
	使得目标 “视为等同于在当前作用域定义”
...
public:			  //派生类的对象就可以访问基类的proteced成员变量和public成员函数
...
\subsubsection{别名声明}
通过using指定别名, 作用等同typedef

\subsubsection{别名模板}
using能够使用模板, 而typedef需要通过结构体包装
\begin{lstlisting}[xleftmargin=2em,xrightmargin=2em]
...
使用typedef加结构体包装的形式实现别名模板存在一个问题, 在使用此别名模板时若没有指定模板参数而是
	传入另一个模板参数, 编译器会无法确认结构体中的这个type是不是类型.

template内出现的名称如果相依于某个template参数, 称之为从属名称. 如果从属名称在class内成嵌套状, 
	称为嵌套从属名称, 即无法确认为类型还是变量的名称

C++规定在嵌套从属名称前加typename关键字以确定该名称是一个类型.
\begin{lstlisting}[xleftmargin=2em,xrightmargin=2em]
...
特例: 请使用关键字typename标识嵌套从属名称, 但不能在基类列(base class lists)和
	成员初值列(member initialization list)中用typename修饰基类(base class), 
	这是确定的一个类型.
...
\subsection[\tt{range for}\quad\quad $\mathbb{BASIC}$]{\color{purple}{范围循环}}
在C++11中, 凡是支持begin,end方法的对象都可以使用范围循环, 本质上是等于迭代器遍历, 只不过细节被隐藏使得看上去更简洁.
TODO:迭代器理解偏差

\begin{lstlisting}[xleftmargin=2em,xrightmargin=2em]
	vector<int>v;  | = for(auto it = begin(v); it != end(v); ++it)
	for(auto n:v); |		auto n = *it;
\end{lstlisting}

\subsubsection{\tt{begin}和\tt{end}}
begin和end方法对内置数组类型进行了重载, 对其它对象则由对象自身实现相关的迭代器方案, 常见的标准库容器string, vector都
	支持.begin().end()方法.

\begin{lstlisting}[xleftmargin=2em,xrightmargin=2em]
	template <class Tp, size_t Np>
	Tp* begin(Tp (&array)[Np]){
		return array;
	}
	template <class Tp, size_t Np>
	Tp* end(Tp (&array)[Np]){
...
\end{lstlisting}
\subsubsection{数组指针}
内置数组类型与指针很相似, 在很多情况下会退化成一个指针, 失去一部分信息, 无法再使用begin和end获得头尾指针
...
\subsection[\tt{cast}\quad\quad $\mathbb{BASIC}$]{\huge\color{purple}{强制类型转换}}
C++引入新的强制类型转换机制, 主要是为了克服C语言强制类型转换没有从形式上体现转换功能和风险不同的问题.

C语言的类型转换, 编译器会在下面这些C++的转换方法去逐一评估, 直到挑选到合适的. 

如果想清晰确认每一步转换是怎么进行的, 使用C++式转换, 对于简单的, 嫌麻烦可由编译器代劳使用C式转换.
...
\begin{paracol}{2}
	\begin{leftcolumn}
		只能用于删除运算对象的底层const, 也只有这个函数可以修改对象的底层const属性.

		即将一个指向常量的指针转换成普通指针, 获得修改指向对象的权限, 此方法只适用于指向对象是非const对象, 
			若指向对象是const则行为未定义.
	\end{leftcolumn}	
	\begin{rightcolumn}
		\begin{lstlisting}[xleftmargin=2em,xrightmargin=2em]
...
\begin{paracol}{2}
	\begin{leftcolumn}
		...
		自由程度高, 可以将int*指针转成string*, 至于之后引发的错误需由自己承担.
...
\begin{lstlisting}[xleftmargin=2em,xrightmargin=2em]
	...
	void fdiff(){
		...
		//pb:0x12be068f8-2
		//pbs:0x12be068f8-2
		//pbr:0x12be068f0-1065353216
		//pbd:0x12be068f8-2
	}
\end{lstlisting}

reinterpret\_cast转换后的B类指针指向原先C类指针pc指向的类内首地址也是类A的首地址.
